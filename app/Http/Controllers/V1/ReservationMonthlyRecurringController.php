...
class ReservationMonthlyRecurringController extends Controller
{
    ...
    protected function storeReservation($request)
    {
        ...
        foreach ($initDates as $date) {
            while ($date->lte($endDate)) {
                $timeDetails = $this->createTimeDetails($date->nthOfMonth($request->week, $request->days[0]), $request->from, $request->to);
